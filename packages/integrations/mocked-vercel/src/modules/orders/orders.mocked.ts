...
/**
 * Local mocked orders data — mirrors the mocked integration's random order generation
 * so that the demo app has background data to browse.
 */
...
const SHIPPING_METHODS = [
    { id: 'SHIP-001', name: 'Standard Shipping', description: '3-5 business days', price: 10 },
    { id: 'SHIP-002', name: 'Express Shipping', description: '1-2 business days', price: 20 },
    { id: 'SHIP-003', name: 'Next Day Shipping', description: 'Next business day', price: 30 },
];
...
// Use seeded random to produce deterministic results across serverless invocations
...
const getRandomInt = (min: number, max: number): number => Math.floor(random() * (max - min + 1)) + min;
...
const generateOrderItem = (itemIndex: number): Orders.Model.OrderItem => {
    ...
    return {
        ...
        quantity,
        price: { value: price, currency: product.currency as Models.Price.Currency },
        total: { value: total, currency: product.currency as Models.Price.Currency },
        subtotal: { value: total - total * 0.23, currency: product.currency as Models.Price.Currency },
        unit: 'PCS' as Orders.Model.OrderItem['unit'],
        currency: product.currency as Models.Price.Currency,
        product: {
            id: product.id,
            sku: product.sku,
            name: product.name,
            ...
            price: { value: product.price, currency: product.currency as Models.Price.Currency },
...
const generateOrder = (orderIndex: number, count: number, getRandomDate: () => Date): Orders.Model.Order => {
    ...
    updateDate.setHours(updateDate.getHours() + getRandomInt(1, 48));
    const paymentDueDate = new Date(orderDate);
    paymentDueDate.setDate(paymentDueDate.getDate() + getRandomInt(1, 5));
    ...
    const discountValue = Math.round(subtotal * 0.1 * 100) / 100;
    const total = subtotal + shippingCost - discountValue;

    const currency = (items[0]?.currency || 'USD') as Models.Price.Currency;
    ...
    return {
        ...
        shippingTotal: { value: shippingCost, currency },
        shippingSubtotal: { value: shippingCost, currency },
        discountTotal: { value: discountValue, currency },
        currency,
        paymentStatus,
        status,
        ...
        shippingMethods: [
            {
                ...
                total: { value: shippingMethod.price, currency },
            },
        ],
        shippingAddress: {
            country: 'US',
...
// Deterministic mock orders (seeded random ensures consistency across serverless invocations)
...
export const mapOrder = (options: Orders.Request.GetOrderParams): Orders.Model.Order | undefined => {
    ...
    const items = { ...order.items, data: [...order.items.data] };
    ...
    if (sort) {
        ...
        items.data.sort((a, b) => {
            ...
            if (field === 'discountTotal' || field === 'total' || field === 'subtotal' || field === 'price') {
                ...
                const aNum = (aValue as Models.Price.Price).value;
                const bNum = (bValue as Models.Price.Price).value;
...
export const mapOrders = (options: Orders.Request.GetOrderListQuery, customerId: string): Orders.Model.Orders => {
    ...
    const [field, direction] = sort?.split('_') || ['createdAt', 'DESC'];
    const isAscending = direction === 'ASC';
